...
- Installe `intelephense`.

**Configuration** :
- Désactive les suggestions PHP par défaut pour éviter les conflits :
```json
...
Pourquoi ? : Indispensable pour coder efficacement bonjour.php, index.php, ou tout exercice des Parties 2, 3, 4.

### 2. PHP Debug

**Description** : Permet de déboguer ton code PHP avec `Xdebug` directement dans VS Code. Idéal pour trouver les erreurs dans tes fichiers PHP.
...
**Pourquoi ?** : Simplifie le débogage pour les débutants, évitant les `var_dump()` ou `die()`. Parfait pour le projet en groupe (Partie 5).
...
- Recherche `PHP DocBlocker`.

**Pourquoi ?** : Simplifie la documentation, un atout pour les exercices et le projet.
